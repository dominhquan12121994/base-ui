<template>
  <div class="card">
    <div class="card-body">
      <h4 class="card-title text-center">Chỉnh sửa nhóm thuộc tính</h4>
      <div v-show="loadingAttributeGroup" class="row">
    ...
    <div class="card-footer">
      <div class="row justify-content-end">
        <button type="submit" class="btn btn-outline-primary" @click="updateAttributeGroup">
          <span v-if="loadingUpdateNhomThuocTinh" class="spinner-border spinner-border-sm mr-2" role="status"
...
export default {
  data() {
    return {
      ...
      loadingUpdateNhomThuocTinh: false,
      loadingLoaiSanPham: true,
      loadingAttributeGroup: true,
      maNhomThuocTinh: '',
      tenNhomThuocTinh: '',
      listLoaiSanPham: [],
  ...
  beforeCreate() {
    ...
    let currentAttributeGroupId = this.$route.params.id;
    attributeGroupInfo.getCurrentAttributeGroup(currentAttributeGroupId)
      .then((response) => {
        this.loadingAttributeGroup = false;
        if (!response.success) {
          Swal.fire(
            "Lỗi!",
            "Có lỗi phát sinh vui lòng liên hệ admin",
        ...
        let attributeGroupData = response.data;
        this.maNhomThuocTinh = attributeGroupData.attribute_group_cd;
        this.tenNhomThuocTinh = attributeGroupData.attribute_group_name;
        this.listSelectedLoaiSanPham = attributeGroupData.product_catalogs.map(productCatalog => productCatalog.id);
        this.listThuocTinh = attributeGroupData.product_attributes.map(productAttribute => {
          let listAttributeValue = [];
          if (productAttribute.attribute_floats.length > 0) {
            productAttribute.attribute_floats.map(attributeValue => {
              listAttributeValue.push({ value: attributeValue.value_display_name });
            });
          } else if (productAttribute.attribute_ints.length > 0) {
            productAttribute.attribute_ints.map(attributeValue => {
              listAttributeValue.push({ value: attributeValue.value_display_name });
            });
          } else {
            productAttribute.attribute_varchars.map(attributeValue => {
              listAttributeValue.push({ value: attributeValue.value_display_name });
            });
          }
          return {
            batBuoc: productAttribute.is_require,
            listGiaTriThuocTinh: listAttributeValue,
            loaiThuocTinh: productAttribute.attribute_type_id,
            maThuocTinh: productAttribute.attribute_cd,
            macDinh: productAttribute.is_default,
            tenThuocTinh: productAttribute.attribute_display_name,
          }
        });
      })
      .catch(error => {
  ...
  methods: {
    updateAttributeGroup() {
      this.validateErrors = {};
      this.loadingUpdateNhomThuocTinh = true;
      ...
      let currentAttributeGroupId = this.$route.params.id;
      let attributeGroupInfo = new AttributeGroup();
      attributeGroupInfo.updateAttributeGroup(currentAttributeGroupId, editAttributeGroupData)
        .then(response => {
          this.loadingUpdateNhomThuocTinh = false;
          if (response.success) {
            this.$emit('NhomThuocTinhUpdated');
            Swal.fire(
              'Thành công!',
              'Đã cập nhật nhóm thuộc tính thành công',
              'success'
            ).then(confirm => {
              if (!confirm) {
        ...
        .catch(error => {
          ...
          if (response.errors.code == "E000003") {
            ...
            this.$emit('NhomThuocTinhUpdatedFail');
            Swal.fire(
              'Không thành công!',
    ...
    addValue(indexAttribute) {
      this.listThuocTinh[indexAttribute].listGiaTriThuocTinh.push(
        {
          value: ''
        }
      );
