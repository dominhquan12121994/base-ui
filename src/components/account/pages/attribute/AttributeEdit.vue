  ...
  >
    ...
    <div slot="modal-footer">
      <button type="button"
              ...
              @click="$bvModal.hide('editThuocTinh')"
              data-dismiss="modal"
      >
        Hủy bỏ
      </button>
      
      <button type="submit" class="btn btn-outline-primary" @click="updateAttribute">
        <span v-if="loadingUpdateThuocTinh" class="spinner-border spinner-border-sm mr-2" role="status" aria-hidden="true"></span>
        Cập nhật
      </button>
    </div>
  </b-modal>
...
export default {
  ...
  watch: {
    ...
    thuocTinh() {
      this.maThuocTinh = this.thuocTinh.attribute_cd;
      this.tenThuocTinh = this.thuocTinh.attribute_display_name;
      this.loaiThuocTinh = this.thuocTinh.attribute_type_id;
      this.macDinh = this.thuocTinh.is_default;
      this.batBuoc = this.thuocTinh.is_require;
      this.listGiaTriThuocTinh = this.thuocTinh.attribute_floats.concat(this.thuocTinh.attribute_ints, this.thuocTinh.attribute_varchars).map(thuocTinh => {
        return {
          value: thuocTinh.value_display_name
        }
      });
    }
  },
  methods: {
    updateAttribute() {
      this.loadingUpdateThuocTinh = true;
      let idThuocTinh = this.thuocTinh.id;
      ...
      attribute.updateAttribute(idThuocTinh, updateData)
        .then(response => {
          this.loadingUpdateThuocTinh = false;
          if (response.success) {
            this.$emit('thuocTinhUpdated');
            Swal.fire(
              'Thành công!',
        ...
        .catch(error => {
          ...
          if (response.errors.code == "E000003") {
            ...
            this.$emit('thuocTinhUpdatedFail');
            Swal.fire(
              'Không thành công!',
