                                        ...
                                        <div class="flex-between san-pham" @click="pickCombo(index)" v-for="(sanPham, index) in data.listSanPham" :key="index">
                                            ...
                                            <div class="flex-between box-right">
                                                ...
                                                <div class="content-right">
                                                    <p>{{ sanPham.retail_entity_price.prices }}</p>
                                                    <p>Số lượng: <b>1</b></p>
                                                </div>
                                            </div>
                                        </div>
...
export default {
    ...
    mounted() {
        ...
        this.sanPham.combos.forEach(combo => {
            let sanphamCombo = {};
            sanphamCombo.so_luong = combo.quantity;
            sanphamCombo.prices = combo.prices;
            sanphamCombo.errors_so_luong = '';
            sanphamCombo.errors_prices = '';
            sanphamCombo.wholesale_prices = this.sanPham.retail_product_entities[0].retail_entity_price.old_wholesale_prices;
            sanphamCombo.cost_prices = this.sanPham.retail_product_entities[0].retail_entity_price.old_cost_prices;
            sanphamCombo.product_avatar = combo.product_entitie.product.product_avatar;
            let varchar_name = '';
            let float_name = '';
            let int_name = '';
            combo.product_entitie.entity_attribute_varchar.forEach(value => {
                varchar_name += ' - ' + value.varchar_value;
            });
            combo.product_entitie.entity_attribute_float.forEach(value => {
                float_name += ' - ' + value.float_value;
            });
            combo.product_entitie.entity_attribute_int.forEach(value => {
                int_name += ' - ' + value.int_value;
            });
            sanphamCombo.product_name = combo.product_entitie.product.product_display_name + float_name + int_name + varchar_name;
            sanphamCombo.id = combo.product_entitie.id;
    ...
    methods: {
        ...
        pickCombo(index) {
            ...
            sanphamCombo.prices = this.data.listSanPham[index].retail_entity_price.prices;
            sanphamCombo.errors_so_luong = '';
            sanphamCombo.errors_prices = '';
            sanphamCombo.wholesale_prices = this.data.listSanPham[index].retail_entity_price.wholesale_prices;
            sanphamCombo.cost_prices = this.data.listSanPham[index].retail_entity_price.cost_prices;
