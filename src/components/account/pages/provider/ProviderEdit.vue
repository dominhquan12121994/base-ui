<template>
  <b-modal
    ...
    @hide="clearModal"
  >
    <div v-if="loadingNhaCungCap" class="text-center">
      <span class="spinner spinner-primary"></span>
    </div>
    <div v-if="!loadingNhaCungCap && nhaCungCap" class="px-5">
      <div class="row my-5">
    ...
    <div slot="modal-footer">
      ...
      <button v-else
              ...
              class="btn"
              @click="changeStatus"
              :class="{ 'btn-outline-danger': nhaCungCap.active_status == 'active', 'btn-outline-success': nhaCungCap.active_status == 'inactive' }"
              v-show="!loadingNhaCungCap"
      >
        <span>{{ nhaCungCap.active_status == 'active' ? 'Inactive' : 'Active' }}</span>
      </button>
      
      <button type="button"
              class="btn btn-outline-secondary"
              @click="$bvModal.hide('editNhaCungCap')"
              data-dismiss="modal"
      >
        Hủy bỏ
      </button>
      
      <button type="submit" class="btn btn-outline-primary" @click="updateProvider">
        <span v-if="loadingUpdateNhaCungCap" class="spinner-border spinner-border-sm mr-2" role="status" aria-hidden="true"></span>
        Cập nhật
      </button>
    </div>
  </b-modal>
...
export default {
  ...
  data() {
    return {
      ...
      loadingNhaCungCap: true,
      loadingChangeTrangThai: false,
      loadingUpdateNhaCungCap: false
    }
  },
  watch: {
    id() {
      this.loadingNhaCungCap = true;
      ...
      let promiseCurrentInfo = new Promise((resolve, reject) => {
        provider.show(this.id)
          .then(response => {
            resolve(response);
          })
          .catch(error => {
      ...
      Promise.all([promiseCreateInfo, promiseCurrentInfo, promiseListProvince]).then((arrResponse) => {
        let responseCreateInfo = arrResponse[0];
        let responseCurrentInfo = arrResponse[1];
        let responseListProvince = arrResponse[2];
        this.listNhomNhaCungCap = responseCreateInfo.data.list_provider_group;
        this.listToChuc = responseCreateInfo.data.list_store;
        this.nhaCungCap = responseCurrentInfo.data;
        this.nhaCungCap.store_id = this.nhaCungCap.stores.map(store => {
          return store.id;
        });
        this.nhaCungCap.provider_group_id = this.nhaCungCap.provider_groups.map(providerGroup => {
          return providerGroup.id;
        });
        this.listTinhThanhPho = responseListProvince.data;
        provider.getListDistrict(this.nhaCungCap.province_id)
          .then(response => {
        ...
        provider.getListWard(this.nhaCungCap.district_id)
          .then(response => {
  ...
  methods: {
    ...
    updateProvider() {
      this.loadingUpdateNhaCungCap = true;
      this.validateError = {
        store_id: '',
        provider_group_id: '',
        province_id: '',
      ...
      let provider = new Provider();
      provider.update(this.id, this.nhaCungCap)
        .then(response => {
          this.loadingUpdateNhaCungCap = false;
          if (response.success) {
            this.$emit('nhaCungCapUpdated');
            Swal.fire(
              'Thành công!',
        ...
        .catch(error => {
          ...
          if (response.errors.code == "E000003") {
            let validateError = response.data;
            for (let [key, value] of Object.entries(validateError)) {
            ...
            this.$emit('nhaCungCapUpdatedFail');
            Swal.fire(
              'Không thành công!',
    ...
    changeStatus() {
      ...
      if (this.nhaCungCap.active_status == 'active') {
        this.nhaCungCap.active_status = 'inactive'
      } else {
        this.nhaCungCap.active_status = 'active';
      }
      this.validateError = {
        store_id: '',
        provider_group_id: '',
      ...
      let provider = new Provider();
      provider.updateStatus(this.id, this.nhaCungCap)
        .then(response => {
