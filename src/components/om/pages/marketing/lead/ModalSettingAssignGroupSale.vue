  ...
  <transition name="overlay" mode="out-in">
    ...
    >
      ...
      <div class="modal-body">
        ...
        <v-form ref="form">
        <v-row v-show="equal" v-for="(setting, index) in settingItems" :key="index" >
          <v-col class="py-0">
              ...
              >
                ...
                <template v-slot:selection="data">
                  <v-chip
                    ...
                    close
                    @click="data.select"
                    @click:close="removeGroupSale(data.item, setting)"
                  >
                    {{ data.item.name }}
                  </v-chip>
                </template>
...
export default {
  name: "ModalSettingAssignGroupSale",
  data() {
    return {
      equal: true,
      pool: false,
      hours: [],
      modalName: 'modal-setting-assign-group-sale',
  ...
  methods: {
    ...
    addItem() {
      this.settingItems.push({time: [], sale_group: []})
    },
    ...
    removeGroupSale(item, setting) {
      const index = setting.sale_group.indexOf(item.id)
      if (index >= 0) setting.sale_group.splice(index, 1)
    },
    removeItem(index) {
    ...
    beforeClose() {
      this.settingItems = [{time: [], sale_group: []}]
    },
    closeModal() {
    ...
    async acceptIsEqual() {
      let setting_marketing_sale_assign = {
        assign_by_pool: [],
        assign_by_robin_round: this.settingItems
      }
      ...
      if (this.$refs.form.validate()) {
        try {
          ...
          await new Lead().settingMarketingSaleAssign({group_assign_settings: setting_marketing_sale_assign})
          this.closeModal()
