...
export default {
  ...
  methods: {
    ...
    async save() {
      if (this.$refs.form.validate()) {
        try {
          this.loadingStart()
          await this.paymentVoucher.updateAsync()

          this.messageSuccess('Bạn đã cập nhật chứng từ chi thành công')
        } catch (e) {
          this.handleException(e)
        } finally {
          this.loadingClose()
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{title: "Cập nhật chứng từ chi"}]);
  },
}
</script>
