...
export default {
  name: "ModalCreatePaymentAccount",
  data() {
    return {
      modalName: 'modal-create-payment-account',
      title: 'Thêm tài khoản MKT',
  ...
  methods: {
    beforeOpen(event) {
      this.paymentAccount.is_active = 1
    ...
    async save() {
      if (this.$refs.form.validate()) {
        try {
          this.loadingStart()
          await this.paymentAccount.storeAsync()
          this.$emit('store')

          this.$refs.form.reset()
          this.closeModal()
          this.messageSuccess('Bạn đã thêm tài khoản quảng cáo MKT thành công')
        } catch (e) {
