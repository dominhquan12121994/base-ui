...
export default {
  name: 'ModalEditResult',
  data() {
    return {
      modalName: 'modal-edit-result',
      ...
      rules: {
        ...
        task: value => value.length > 0 || 'Nhiệm vụ.',
  ...
  methods: {
    beforeOpen(event) {
      ...
      this.result = _.cloneDeep(event.params.result)
      this.result.task_ids = this.result.tasks.pluck('id')
      this.result.updated_by = this.user.id
    },
    beforeClose() {
    },
    closeModal() {
    ...
    async save() {
      if (this.$refs.form.validate()) {
        try {
          ...
          await this.result.updateAsync()
          this.$emit('update', this.result)

          this.$refs.form.reset()
          this.closeModal()
          this.messageSuccess('Bạn đã cập nhật kết quả thành công')
        } catch (e) {
