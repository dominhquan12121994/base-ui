...
export default {
  ...
  methods: {
    beforeOpen(event) {
      this.task = _.cloneDeep(event.params.task)
      this.task.updated_by = this.user.id
    },
    beforeClose() {
    },
    closeModal() {
    ...
    async save() {
      if (this.$refs.form.validate()) {
        try {
          ...
          await this.task.updateAsync().then((result) => {
            if (result) {
              const task = {
                id: result.id,
                name: result.name,
          ...
          this.closeModal()
          this.messageSuccess('Bạn đã cập nhật nhiệm vụ thành công')
        } catch (e) {
