...
export default {
  name: "ModalCreateOrderCancelReason",
  data() {
    return {
      modalName: 'modal-create-order-cancel-reason',
      title: 'Thêm mới lý do hủy đơn',
  ...
  methods: {
    beforeOpen() {
      this.orderCancelReason.is_active = 1
    },
    beforeClose() {
      this.validateErrors = {
        code: '',
    ...
    async save() {
      if (this.$refs.form.validate()) {
        try {
          ...
          this.loadingStart()
          this.orderCancelReason.created_by = this.user.id
          await this.orderCancelReason.storeAsync()
          this.$emit('store')
          this.$refs.form.reset()
          this.closeModal()
          this.messageSuccess('Bạn đã thêm lý do hủy đơn thành công')
        } catch (e) {
          this.messageWarning('Bạn đã thêm lý do hủy đơn không thành công, vui lòng thử lại')
